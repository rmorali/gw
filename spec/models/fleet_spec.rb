...
describe Fleet do
  ...
  let(:partial_unit) {Factory :fleet}
  ...
  context 'moving' do
    before(:each) do
      ...
      @moving_fleet = unit.move 5, planet

      partial_unit.squad = squad
      partial_unit.quantity = 5
      partial_unit.save
      @carried_unit3 = Factory(:fleet, :carried_by => partial_unit)
      @carried_unit4 = Factory(:fleet, :carried_by => partial_unit)
      @moving_partial_fleet = partial_unit.move 1, planet
    end
    ...
    context 'setting to move' do
      ...
      it 'should drop carried units in a partial moving' do
