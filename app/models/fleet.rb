class Fleet < GenericFleet
  ...
  def self.create_from_facility unit, quantity, planet, squad
    if unit.type == 'CapitalShip'
      ...
      fleet = find_or_create_by_generic_unit_id_and_planet_id_and_squad_id_and_weapon1_id_and_weapon2_id(:generic_unit_id => unit.id, :planet_id => planet.id, :squad_id => squad.id, :fleet_name => '', :weapon1_id => nil, :weapon2_id => nil, :level => 0, :round => Round.getInstance.number)
      if fleet.quantity
