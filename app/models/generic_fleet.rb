...
:message => "no special characteres."
  ...
  def description
    ...
    when 'Trooper'
      ...
      if is_a_builder?
        info << "<br>- Constroi Fabricas (Presenca min: #{Setting.getInstance.minimum_presence_to_construct / Setting.getInstance.presence_to_influence * 100}) "
  ...
  def span_style
    ...
    when 'Warrior'
      style << "border-radius:20%; border:dotted 1px; padding:2px; background-color:#4E342E"
    when 'Sensor'

    when 'Trooper'
      style << "font-weight:bolder; background-color:#4E342E"
    when 'Commander'
      style << "outline:solid"
    else
